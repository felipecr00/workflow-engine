...
export async function createFolder(name: string, parentId?: string | null): Promise<FolderInfo> {
...
export async function saveProject(
  id: string,
  updates: { bpmnXml?: string; name?: string; deployedDefinitionId?: string | null },
): Promise<ProjectInfo> {
  const res = await fetch(`${BASE}/projects/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function listUserTasks(filter: {
  instanceId?: string;
  state?: string;
  assignee?: string;
...
export async function claimUserTask(id: string, claimedBy: string): Promise<void> {
...
export async function completeUserTask(id: string, variables?: Record<string, unknown>): Promise<void> {
